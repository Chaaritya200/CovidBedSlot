...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:root@localhost/coviddata'
db=SQLAlchemy(app)
...
def slotbooking():
    ...
    if request.method == "POST":
        ...
        if dbb:
            seat = 0  # Initialize seat variable
            ...
            # Commit the seat reduction if seat is available
            if seat > 0:
                db.session.commit()
            else:
